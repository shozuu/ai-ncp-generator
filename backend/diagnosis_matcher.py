...
class VectorDiagnosisMatcher:
    ...
    def format_assessment_for_embedding(self, assessment_data: Dict) -> str:
        ...
        if phys_exam or phys_exam_other:
            # Remove prefixes like "Respiratory:" for conciseness
            exam_items = []
            for item in phys_exam:
                # Extract just the finding (e.g., "Crackles" from "Respiratory: Crackles")
                if ':' in item:
                    exam_items.append(item.split(':', 1)[1].strip())
                else:
    ...
    async def select_best_diagnosis(
        ...
        try:
            ...
            ai_prompt = f"""
                ...
                {{
                    "diagnosis": "exact diagnosis name from candidate list",
                    "definition": "definition from selected candidate",
                    "defining_characteristics": ["array from selected candidate"],
                    "related_factors": ["array from selected candidate"], 
                    "risk_factors": ["array from selected candidate"],
                    "suggested_outcomes": ["array from selected candidate"],
                    "suggested_interventions": ["array from selected candidate"],
            ...
            logger.info(f"Raw AI selection response: {raw_response}")
            ...
            if start_brace != -1 and end_brace != -1:
                ...
                logger.info(f"AI selected diagnosis: {result.get('diagnosis')}")
                return result
            else:
