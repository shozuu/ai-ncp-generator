...
export const manualModeSchema = z
  .object({
    ...
    general_condition: z
      ...
      .refine(value => !value || value.trim().length <= 500, {
        message:
          'General condition description should not exceed 500 characters',
      }),
    ...
    heart_rate_bpm: z
      ...
      .refine(
        value => !value || (Number(value) >= 30 && Number(value) <= 250),
        {
    ...
    cephalocaudal_assessment: z
      .object({
        ...
        nose_and_sinuses: z
          ...
          .refine(value => !value || value.trim().length <= 300, {
            message:
              'Nose and sinuses findings should not exceed 300 characters',
          }),
        mouth_and_throat: z
          .string()
          .optional()
          .refine(value => !value || value.trim().length <= 300, {
            message:
              'Mouth and throat findings should not exceed 300 characters',
          }),
        neck: z
          .string()
          .optional()
        ...
        chest_and_lungs: z
          ...
          .refine(value => !value || value.trim().length <= 300, {
            message:
              'Chest and lungs findings should not exceed 300 characters',
          }),
        heart: z
          .string()
          .optional()
  ...
  .refine(
    data => {
      // Check if at least one clinical data field is provided
      const hasGeneralCondition = data.general_condition?.trim()?.length > 0
      const hasSubjective = data.subjective?.trim()?.length > 0
      const hasObjective = data.objective?.trim()?.length > 0
      const hasVitalSigns = !!(
        data.heart_rate_bpm ||
        data.blood_pressure_mmhg?.trim() ||
      ...
      const hasHistory = !!(
        data.onset_duration?.trim() ||
        data.severity_progression?.trim() ||
        data.medical_impression?.trim() ||
        (data.associated_symptoms && data.associated_symptoms.length > 0) ||
        data.other_symptoms?.trim()
      )
      const hasMedicalHistory = !!(
        (data.medical_history && data.medical_history.length > 0) ||
        data.other_medical_history?.trim()
      )
      const hasPhysicalExam = !!(
        data.height ||
        data.weight ||
        data.laboratory_results?.trim() ||
        (data.cephalocaudal_assessment &&
          Object.values(data.cephalocaudal_assessment).some(v => v?.trim()))
      )
    ...
    {
      message:
        'Please provide at least one of the following: Chief Complaint (General Condition), Subjective Data, Objective Data, Vital Signs, History of Present Illness, Medical History, or Physical Examination findings.',
